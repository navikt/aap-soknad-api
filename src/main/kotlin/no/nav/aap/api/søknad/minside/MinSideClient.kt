            ...
            if (enabled) {
                if (!repos.utkast.existsByFnrAndSkjematype(fnr.fnr, skjemaType)) {
                    if (sendenabled) {
                        ...
                        utkast?.dec().also { log.info("Mellomlagring counter $it etter opprettet $fnr") }
                        repos.utkast.save(Utkast(fnr.fnr, eventId, CREATED))
                    }
                }
                else {
            ...
            if (enabled) {
                repos.utkast.findByFnrAndSkjematype(fnr.fnr,skjemaType)?.let { u ->
                    if (sendenabled) {
                        ...
                        repos.utkast.delete(u)
                        utkast?.decrementAndGet().also { log.info("Mellomlagring counter $it etter avsluttet $fnr") }
                    }
