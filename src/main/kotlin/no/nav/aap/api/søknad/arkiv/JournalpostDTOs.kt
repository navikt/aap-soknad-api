...
import no.nav.aap.api.søknad.arkiv.ArkivJournalpostGenerator.Companion.ROUTING
...
data class Journalpost(
    ...
    data class DokumentVariant private constructor(val filtype : String, val fysiskDokument : String, val variantformat : String) {
        constructor(fysiskDokument : String, variantformat : VariantFormat = ARKIV, filtype : Filtype = PDFA) : this(filtype.name,
            fysiskDokument,
            variantformat.name)
    ...
    override fun toString() =
        "${javaClass.simpleName} [tittel=$tittel,dokumenter=$dokumenter,eksternReferanseId=$eksternReferanseId, tilleggsopplysninger=$tilleggsopplysninger]"
}