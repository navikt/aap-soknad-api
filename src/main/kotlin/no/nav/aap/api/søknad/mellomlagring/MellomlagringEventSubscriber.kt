            ...
            subscriber.subscribe(subscription.navn) { event ->
                ...
                with(event.pubsubMessage) {
                    ...
                    metadata(mapper)?.let {
                        ...
                        when (type) {
                            OBJECT_FINALIZE -> dittNav.opprettUtkast(it.fnr, "Du har en påbegynt ${it.type.tittel.decap()}", it.type, it.eventId)
                            OBJECT_DELETE -> if (endeligSlettet()) { dittNav.avsluttUtkast(it.fnr, it.type) } else { Unit }
